...
def draw_line(canvas, y1, x1, y2, x2, col):
    ...
    line = canvas.create_line(x1+80, 700 - y1, x2+80, 700 - y2, fill=col)
    canvas.itemconfig(line, tags=('line'))
    return line
...
def draw_all_edges(ROOT, canvas, neighbors, map_locations):
    ...
    for neighbor in neighbors:
        for neighbor_of in neighbors[neighbor]:
            ...
            lines.append(draw_line(canvas, y1, x1, y2, x2, 'white'))
    return lines
...
def reset():
    # ROOT.update_idletasks()
    canvas.delete('line')
    # canvas.create_rectangle(0, 0, 1200, 600, fill="black")
    new_map = PhotoImage(file='map.png')
    image_map1 = canvas.create_image(0, 0, image=new_map, anchor='nw')
    lines = draw_all_edges(ROOT, canvas, neighbors, map_locations)
    # ROOT.update()
...
scale = Scale(ROOT, orient=HORIZONTAL, length=100, width=20,
...
y.place(x=110, y=600)
z.place(x=220, y=600)
speedLabel.place(x=300, y=600)
scale.place(x=400, y=600)
mapReset.place(x=600, y=600)
doProcess.place(x=650, y=600)

map = PhotoImage(file='map.png')
image_map = canvas.create_image(0, 0, image=map, anchor='nw')
lines = draw_all_edges(ROOT, canvas, neighbors, map_locations)
ROOT.update()
ROOT.mainloop()
