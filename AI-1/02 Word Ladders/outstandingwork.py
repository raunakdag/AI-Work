...
def find_words(words_file):
    ...
    for first_word_index in range(len(all_words)):
        first_word = all_words[first_word_index]
        if first_word not in pairs:
            pairs[first_word] = []

        for second_word_index in range(first_word_index + 1, len(all_words)):
            second_word = all_words[second_word_index]

            if second_word not in pairs:
                pairs[second_word] = []

            # One Letter Different
            if letters_different(first_word, second_word):
                pairs[first_word].append(second_word)
                pairs[second_word].append(first_word)
...
def letters_different(first, second):
...
word_relations = 'temp'
with open("data_outstanding_work.pkl", "rb") as infile:
